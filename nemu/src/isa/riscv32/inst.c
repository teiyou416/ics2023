...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)                   \
    ...
    INSTPAT("??????? ????? ????? ??? 00000 11011 11", j, J,
            s->dnpc = s->pc + imm);
    INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal, J,
            s->dnpc = s->pc + imm, R(rd) = s->pc + 4, CALL(s->pc, s->dnpc));
    ...
    INSTPAT("0000000 00001 ????? 011 ????? 00100 11", seqz, I,
            R(rd) = src1 < 1 ? 1 : 0);
    ...
    INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr, I,
            s->dnpc = (src1 + imm) - ((src1 + imm) & 1), R(rd) = s->pc + 4,
            CALL(s->pc, s->dnpc));
    ...
    INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu, R,
            R(rd) = ((int64_t)(int32_t)src1 * (int64_t)src2) >> 32);
    ...
    //         R(rd) = CSR(imm), CSR(imm) = CSR(imm) | src1);
    // INSTPAT("??????? ????? ????? 011 ????? 11100 11", csrrc, I,
    //         R(rd) = CSR(imm), CSR(imm) = CSR(imm) & (~src1));

    // // Trap-Return Instructions
    // INSTPAT("0011000 00010 00000 000 00000 11100 11", mret, N,
    //         s->dnpc = CSR(MEPC), RET(s->pc, s->dnpc),
    //         CSR(MSTATUS) = ((CSR(MSTATUS) & MPIE) ? CSR(MSTATUS) | MIE
    //                                               : CSR(MSTATUS) & (~MIE)) |
    //                        MPIE);

    INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall, N,
            s->dnpc = INTR(0xb, s->pc),
            CALL(s->pc, s->dnpc)); // 0xb is Machine Mode Environment call
